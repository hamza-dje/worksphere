  ...
  return (
    <div className={"flex justify-between min-w-[500px] not-last:mb-3"}>
      ...
      <div className={"flex flex-col max-w-1/3 min-w-1/5"}>
        ...
        <span className={"text-xs line-clamp-3 opacity-60"}>{description}</span>
      </div>
      {/* Skills */}
      <div className={"flex flex-col"}>
        <span className={"font-primary text-xs mb-0.5"}>Skills:</span>
        {skills.map((skill, i) => (
          <div key={i} className="font-primary text-[11px] opacity-40">
            {skill}
          </div>
        ))}
      </div>
      <div className={"grid grid-cols-[auto_auto] gap-x-2"}>
        ...
        <span className={"font-primary font-bold text-xs text-green text-end"}>
          +{cost}
        </span>
      ...
      <div className={"flex flex-col items-center gap-2"}>
        <OverlayButton
          ...
          openOverlayButton={
            <button className={"normal-button text-xs px-3.5 py-2 rounded-xl"}>
              Modify
            </button>
          }
          overlayContent={
          ...
          confirmButton={
            <button className={"normal-button"}>Finish modifying</button>
          }
        />
        <OverlayButton
          openOverlayButton={
            <button
              className={"stroke-button text-xs px-3.5 py-1.5 rounded-xl"}
            >
              Delete
            </button>
          }
          overlayContent={
            <div className={"font-primary font-bold text-center mb-10"}>
              You sure you want to delete your service?
            </div>
          }
          cancelButtonContent={"No, keep it!"}
          confirmButton={
            <button className={"normal-button bg-red"}>Yes, delete!</button>
          }
        />
      </div>
    </div>
