...
export default function MessagesContainer({
    ...
    messages: Array<{ messageFrom: "me" | "him", message: string, timestamp: string }>
}) {
    return (
                ...
                messages.map((message, i) => {
                    ...
                    useEffect(() => {
                        if (messageContainerRef.current && messageRef.current && seenTimeRef.current) {
                            if (seenTimeShown) {
                                messageContainerRef.current.style.height = `${messageRef.current.offsetHeight + seenTimeRef.current.offsetHeight + 4}px`;
                            } else {
                    ...
                    const lastMessage: boolean = messages[i + 1]?.messageFrom !== message.messageFrom && messages[i - 1]?.messageFrom === message.messageFrom;
                    const messageInMiddle: boolean = messages[i + 1]?.messageFrom === message.messageFrom && messages[i - 1]?.messageFrom === message.messageFrom;
                    const userChanged: boolean = i !== messages.length - 1 && message.messageFrom !== messages[i + 1]?.messageFrom;
                    ...
                    >
                        {senderPicShown && <div className="w-6 aspect-square bg-primary rounded-full self-end" />}
                        <div
                            ref={messageRef}
