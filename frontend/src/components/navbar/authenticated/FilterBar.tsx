...
export default function FilterBar() {
  ...
  return (
    <div className="sticky z-9 max-md:w-[calc(100%-16px)] mx-auto sm:h-[60px] group top-[84px]">
      <div
        className={`max-sm:absolute ${
          makeVisible || filterFixed
            ? "sm:-translate-y-4 sm:max-xl:-translate-y-5"
            : "sm:opacity-0 sm:-translate-y-full"
        } sm:group-hover:-translate-y-4 sm:max-xl:group-hover:-translate-y-5 max-sm:-translate-y-5 sm:group-hover:opacity-100 max-sm:w-full max-sm:flex max-sm:flex-col max-sm:gap-4 transition-[top_translate_opacity] duration-[250ms] ease-out ${
          filterShown ? "max-sm:top-0" : "max-sm:-top-[320px]"
        }`}
      >
        <form
          onSubmit={handleSubmit}
          className={`bg-white p-6 relative !pt-10 max-xl:p-5 max-sm:flex-col flex w-[1200px] max-xl:w-[945px] max-xl:gap-2.5 max-lg:w-[calc(100%-64px)] max-md:w-full mx-auto shadow-[0_0_27px_rgba(0,0,0,0.08)] rounded-b-[36px] gap-4 [&_input]:placeholder:text-[16px] [&_input,&_select]:px-4 [&_input,&_select]:py-3.5 ${
            !filterShown && "max-sm:opacity-0 max-sm:pointer-events-none"
          }`}
        >
          <button
            type="button"
          ...
          >
            <svg
              ...
              className={`stroke-black opacity-40 ${
                filterFixed ? "fill-black" : "fill-none"
              } size-4`}
            >
              <path
                d="M1 17L6.5 11.5M6.5 11.5L3 8L7 7L9 5L10 1L17 8L13 9L11 11L10 15L6.5 11.5Z"
                strokeLinecap="round"
          ...
          <div className="flex flex-1 min-w-[20%] [&>div>input]:not-placeholder-shown:font-primary [&>div>input]:not-placeholder-shown:font-bold [&>div]:w-[120px] max-xl:[&>div]:max-w-fit max-sm:[&>div]:max-w-full max-xl:[&>div]:flex-shrink [&>div>label]:absolute [&>div>label]:font-primary [&>div>label]:text-lg [&>div>label]:font-bold [&>div>label]:top-1/2 [&>div>label]:-translate-y-1/2 [&>div>label]:right-4">
            <div className="relative min-w-0 flex-1 -mr-[1px]">
              ...
              <label
                htmlFor="min"
                className="peer-placeholder-shown:hidden max-xl:!text-sm"
              >
                $
              </label>
            </div>
          ...
          <button className="big-button px-8 text-[16px] py-4 flex items-center justify-center gap-3 max-xl:text-sm max-xl:py-3.5 max-xl:px-6">
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 21 22"
              className="size-4 max-xl:size-3 stroke-white"
            >
              <path
                d="M2 4H7.49994H12.9999V2V6M8 11H18.9999M8 11V9M8 11V13M16.9999 4H18.9999M4 11H2M12.9999 18H2M12.9999 18V16M12.9999 18V20M16.9999 18H18.9999"
                strokeWidth="3"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
