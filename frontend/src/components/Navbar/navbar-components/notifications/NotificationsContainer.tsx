...
export default function NotificationContainer({
    ...
    return (
        <div className={`absolute sm:left-1/2 sm:-translate-x-1/2 sm:top-[74px] sm:w-[320px] sm:max-md:w-[300px] max-sm:w-full max-sm:top-[calc(100%+10px)] max-sm:left-0 max-sm:px-2 flex flex-col gap-1 transition-all duration-200 ease-out ${!notificationsShown && '-translate-y-5 opacity-0 pointer-events-none'}`}>
