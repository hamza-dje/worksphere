...
export default function Navbar() {
    ...
    return (
        <header className="z-10 role-freelancer w-full py-5 shadow-[0_0_27px_rgba(0,0,0,0.08)] rounded-b-[36px] flex justify-between px-[140px] max-lg:px-[60px] max-sm:px-6 sticky top-0 right-0 left-0 bg-white">
            <Link href="/">
                <div className="flex gap-3.5 items-center">
                    ...
                    <span className="font-bold font-primary bg-gradient-to-r from-green to-blue bg-clip-text text-transparent text-[27px] max-sm:hidden">
                        Work<span className="font-black font-primary text-transparent">Wave</span>
                    </span>
                </div>
            </Link>
            ...
            <div className="flex items-center gap-5">
                <span className="relative">
                    <img
                        ...
                        className="h-[30px]"
                    />
                    <span className="text-white bg-red absolute top-0 -translate-y-[50%] right-0 text-xs font-primary font-medium px-1 translate-x-2 border-3 border-white rounded-full">2</span>
                </span>
                ...
                <div className="relative">
                    <div className="flex items-center gap-[18px] cursor-pointer" onClick={() => setUserMenuShown(u => !u)}>
                        <div className="w-[44px] aspect-square rounded-full bg-primary"></div>
                        <div className="flex flex-col max-md:hidden">
                            <span className="font-primary font-bold text-sm text-primary">Hamza Djedidi</span>
                            <span className="text-xs text-primary">Graphic Designer</span>
                        </div>
                    </div>

                    <div className={`absolute top-[80px] lg:right-[50%] max-lg:right-0 lg:translate-x-[50%] w-[360px] grid grid-cols-2 gap-x-2.5 max-sm:pl-12 max-sm:w-screen ${userMenuShown ? 'gap-y-2.5' : 'gap-y-0 pointer-events-none -translate-y-5 opacity-0'} transition-all duration-200 ease-out`}>
                        ...
                        <button className="normal-button col-span-2 my-2">Post a service</button>
                        <MenuButton
                            name="Get Support"
